...
殿の依頼を将軍に振る前に、以下を毎回チェックせよ:

1. **殿が明示していない機能を入れていないか**
   - 「Firestore使う」≠「Auth必須」
   - 「PWA化」≠「Push通知/オフライン同期/バックグラウンド更新フル装備」
   - 「スマホで使う」≠「マルチユーザー想定」
2. **既存類似プロジェクトの運用パターンを先に grep**
   - 殿の他プロジェクト（camp-schedule-app, fumoto, line-ai-friend 等）が同様の用途で何を選んだか必ず確認
   - 例: camp-schedule-app は Firestore あり / Auth なし / apiKey ハードコード = 殿の標準パターン
3. **殿の手作業を増やす設計になっていないか**
   - 「殿が手元で○○してください」が >5分発生する設計は過剰実装の兆候
   - 認証・環境変数化・承認ドメイン手作業は **殿1人運用なら基本不要**
4. **「将来必要かも」「ベストプラクティス」「セキュリティ強化のため」を理由にしていないか**
   - これらは過剰実装の典型的な口実
   - 殿が明示要求するまで入れるな

詳細・教訓: `~/.claude/projects/.../memory/feedback_no_unrequested_features.md`
（2026-04-27 weight-diary 過剰実装インシデント、4-6h の手戻り発生）
